        ...
        try {
            ...
            if (!$archivo || !$archivo->isValid()) {
                return response()->json([
                    'error' => 'Archivo no válido o corrupto'
                ], 400);
            }

            // Crear el directorio si no existe
            $directory = 'reports/sunat';
            ...
            // Guardar el archivo usando Laravel's file storage
            $path = $archivo->storeAs($directory, $filename);
            
            // Obtener la ruta completa del archivo
            $fullPath = Storage::path($path);
            
            // Debug: Log the path information
            ...
            // Verificar que el archivo se guardó correctamente y es un archivo
            if (!file_exists($fullPath)) {
                return response()->json([
                    'error' => 'No se pudo guardar el archivo',
                    'path_attempted' => $fullPath
                ], 500);
            }

            if (!is_file($fullPath)) {
                return response()->json([
                    'error' => 'La ruta no corresponde a un archivo válido',
                    'path' => $fullPath,
            ...
            if (!is_readable($fullPath)) {
                return response()->json([
                    'error' => 'El archivo no tiene permisos de lectura',
                    'path' => $fullPath
                ], 500);
            }
            ...
            // Alternative approach: Use temporary file if Storage::path() doesn't work
            $tempPath = null;
            try {
                // Try parsing with the stored file first
                $parser = new Parser();
                $pdf = $parser->parseFile($fullPath);
            } catch (Exception $e) {
                // If that fails, try creating a temporary file
                $tempPath = tempnam(sys_get_temp_dir(), 'sunat_pdf_');
                if ($tempPath === false) {
                ...
                if (!copy($fullPath, $tempPath)) {
                    throw new Exception('No se pudo copiar el archivo al directorio temporal');
                }
                
                // Try parsing the temporary file
            ...
            if (count($pages) < 5) {
                ...
                return response()->json([
                    'error' => 'El PDF no tiene el formato esperado (mínimo 5 páginas requeridas)',
            ...
            // Verificar que se encontraron todos los índices necesarios
            foreach ($indices as $key => $valor) {
                if ($valor === false) {
                    Storage::delete($path);
                    if ($tempPath) unlink($tempPath);
                    return response()->json([
                        'error' => "No se encontró la sección '$key' en el PDF. Verifique que el formato del archivo sea correcto."
                    ], 400);
                }
            }
            ...
            if ($i0_pagina_2 === false || $i1_pagina_2 === false) {
                Storage::delete($path);
                if ($tempPath) unlink($tempPath);
                return response()->json([
                    'error' => 'No se encontró la información económico-financiera en la página 2'
                ], 400);
            }
            ...
            // Verificar que hay suficientes líneas en la página 2
            if (count($pagina_2) < 19) {
                Storage::delete($path);
                if ($tempPath) unlink($tempPath);
                return response()->json([
                    'error' => 'La página 2 no tiene el formato esperado'
                ], 400);
            }
            ...
            // Continuar con el procesamiento del resto de páginas...
            $text = $pdf->getText();
            $text = str_replace("\n", ' ', $text);
            ...
            if ($i1_text === false || $i2_text === false || $i3_text === false) {
                Storage::delete($path);
                if ($tempPath) unlink($tempPath);
                return response()->json([
                    'error' => 'No se encontró información de declaraciones mensuales'
                ], 400);
            }
